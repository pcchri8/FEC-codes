...
%% parameter_sweep for the requested inputs
...
function [err,data_rates] = parameter_sweep(SNR,d,M,modul_name,Ts)
    if nargin < 5
        Ts = 1;
    end
    ...
    for m = 1:length(M)
        ...
        for j = 1:length(d) 
            ...
            for i = 1:length(modul_name) 
                for k = 1:length(SNR)
                    [err(m,j,i,k,1),err(m,j,i,k,2),err(m,j,i,k,3)] = hamming_sim(SNR(k),M(m),modul_name{i},d(j),54,3e7);%numofbits = 1e8
                end
            end
        end
    end
